...
pygame.init()
"""
rooms this time, same three dimensional maze structure
"""
...
pygame.display.set_caption("come on in and solve that maze")

def get(Q, X, Y, Z): return Q[Z][Y][X]

def carve_room(Q, x, y, z, w, h, l, marked):
    cent = x + (w / 2), y + (h / 2) 
    for Z in range(z, z+l-1):
        for Y in range(y, y+h):
            for X in range(x, x+w):
                marked.append((X, Y, Z))
                exits = get(Q, X, Y, Z)
                if Y < y+h-1: exits[2] = 1
                if Y > y: exits[0] = 1
                if X < x+w-1: exits[1] = 1
                if X > x: exits[3] = 1
                if (X, Y) == cent:
                    if Z > z: exits[4] = 1
                    if Z < z+l-2: exits[5] = 1
...
def first_draft():
    ...
    while heads or len(marked) < (W*H)/2:
        ...
        route = routes[(x, y, z)]
        exits = get(Q, x, y, z)
        carve_room(Q, 2, 2, 2, 6, 6, 6, marked)
