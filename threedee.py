...
ENT = (randint(0, W), randint(0, H), randint(0, L))
EXT = None
...
def solve():
    ...
    EXT = ENT
    for route in routes: EXT = route if routes[route] > routes[EXT] else EXT
    X, Y, Z = ENT
    ...
    while True:
        ...
        SCREEN.blit(draw(Q, Z), ((((SCREEN.get_width() / PW) // 2) - X) * PW, (((SCREEN.get_height() / PW) // 2) - Y) * PW))
